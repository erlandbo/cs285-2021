...
class FFModel(nn.Module, BaseModel):
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        data_stats = {key: ptu.from_numpy(val) for key, val in data_statistics.items()}
        prediction, _ = self.forward(
            obs, acs, data_stats['obs_mean'], data_stats['obs_std'], data_stats['acs_mean'], data_stats['acs_std'],
            data_stats['delta_mean'], data_stats['delta_std']
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        :return:
        """
        obs, acs, obs_next = ptu.from_numpy(observations), ptu.from_numpy(actions), ptu.from_numpy(next_observations)
        data_stats = {key: ptu.from_numpy(val) for key, val in data_statistics.items()}
        target = normalize(obs_next-obs, data_stats['delta_mean'], data_stats['delta_std'])
