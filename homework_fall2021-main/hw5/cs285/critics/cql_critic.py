...
class CQLCritic(BaseCritic):
    ...
    def dqn_loss(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        q_t_values = torch.gather(qa_t_values, dim=1, index=ac_na[:,None]).squeeze()
        qa_tp1_values = self.q_net(next_ob_no)
        if self.double_q: actions = torch.argmax(qa_t_values,dim=1)
        else: actions = torch.argmax(qa_tp1_values)
        qa_tp1 = reward_n + self.gamma * torch.gather(qa_tp1_values, dim=1, index=actions[:,None]).squeeze() * (1-terminal_n)
        loss = self.loss(q_t_values, qa_tp1)
