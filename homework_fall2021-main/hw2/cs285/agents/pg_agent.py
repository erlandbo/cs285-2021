...
class PGAgent(BaseAgent):
    ...
    def estimate_advantage(self, obs, rews_list, q_values, terminals):
        ...
        if self.nn_baseline:
            ...
            values = np.mean(q_values) + (values_unnormalized - np.mean(values_unnormalized)) * np.std(q_values) / np.std(values_unnormalized)
    ...
    def _discounted_return(self, rewards):
        ...
        T = len(rewards)
        list_of_discounted_returns = [np.sum(self.gamma ** np.arange(T) * rewards)] * T
    ...
    def _discounted_cumsum(self, rewards):
        ...
        rewards_discounted = discounts * rewards
        list_of_discounted_cumsums = np.flip(np.cumsum(np.flip(rewards_discounted))) / discounts
